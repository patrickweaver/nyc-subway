// A list of pairs of stationIds, this is the same as /src/data/lineGroupIntervals.js

const lineGroupIntervals = {
  Blue: [
    ["H08", "H09", "Beach 44 St (A)", "Beach 36 St (A)"],
    ["H09", "H10", "Beach 36 St (A)", "Beach 25 St (A)"],
    ["H10", "H11", "Beach 25 St (A)", "Far Rockaway - Mott Av (A)"],
    ["H03", "H04", "Howard Beach - JFK Airport (A)", "Broad Channel (A S)"],
    ["H04", "H06", "Broad Channel (A S)", "Beach 67 St (A)"],
    ["H06", "H07", "Beach 67 St (A)", "Beach 60 St (A)"],
    ["H07", "H08", "Beach 60 St (A)", "Beach 44 St (A)"],
    ["A55", "A57", "Euclid Av (A C)", "Grant Av (A)"],
    ["A57", "A59", "Grant Av (A)", "80 St (A)"],
    ["A59", "A60", "80 St (A)", "88 St (A)"],
    ["A60", "A61", "88 St (A)", "Rockaway Blvd (A)"],
    ["A61", "A63", "Rockaway Blvd (A)", "104 St (A)"],
    ["A63", "A64", "104 St (A)", "111 St (A)"],
    ["A64", "A65", "111 St (A)", "Ozone Park - Lefferts Blvd (A)"],
    //["A51", "A55", "Broadway Jct (A C)", "Euclid Av (A C)"], // EXPRESS
    ["A61", "H02", "Rockaway Blvd (A)", "Aqueduct - N Conduit Av (A)"],
    [
      "H02",
      "H03",
      "Aqueduct - N Conduit Av (A)",
      "Howard Beach - JFK Airport (A)",
    ],
    //["A46", "A48", "Nostrand Av (A C)", "Utica Av (A C)"], // EXPRESS
    //["A48", "A51", "Utica Av (A C)", "Broadway Jct (A C)"], // EXPRESS
    [
      "A41",
      "A42",
      "Jay St - MetroTech (A C F)",
      "Hoyt - Schermerhorn Sts (A C G)",
    ],
    //["A42", "A46", "Hoyt - Schermerhorn Sts (A C G)", "Nostrand Av (A C)"], // EXPRESS
    //["A32", "A34", "W 4 St (A C E)", "Canal St (A C E)"], // EXPRESS
    ["A34", "A36", "Canal St (A C E)", "Chambers St (A C)"],
    ["A36", "A38", "Chambers St (A C)", "Fulton St (A C)"],
    ["A38", "A40", "Fulton St (A C)", "High St (A C)"],
    ["A40", "A41", "High St (A C)", "Jay St - MetroTech (A C F)"],
    //["A24", "A27", "59 St - Columbus Circle (A B C D)", "42 St - Port Authority Bus Terminal (A C E)",], // EXPRESS
    [
      "A27",
      "A28",
      "42 St - Port Authority Bus Terminal (A C E)",
      "34 St - Penn Station (A C E)",
    ],
    //["A28", "A31", "34 St - Penn Station (A C E)", "14 St (A C E)"], // EXPRESS
    ["A31", "A32", "14 St (A C E)", "W 4 St (A C E)"],
    ["A03", "A05", "Dyckman St (A)", "190 St (A)"],
    ["A05", "A06", "190 St (A)", "181 St (A)"],
    ["A06", "A07", "181 St (A)", "175 St (A)"],
    ["A07", "A09", "175 St (A)", "168 St (A C)"],
    //["A09", "A12", "168 St (A C)", "145 St (A C)"], // EXPRESS
    //["A12", "A15", "145 St (A C)", "125 St (A B C D)"], // EXPRESS
    //["A15", "A24", "125 St (A B C D)", "59 St - Columbus Circle (A B C D)"], // EXPRESS
    ["A02", "A03", "Inwood - 207 St (A)", "Dyckman St (A)"],
    ["H04", "H12", "Broad Channel (A S)", "Beach 90 St (A S)"],
    ["H12", "H13", "Beach 90 St (A S)", "Beach 98 St (A S)"],
    ["H13", "H14", "Beach 98 St (A S)", "Beach 105 St (A S)"],
    ["H14", "H15", "Beach 105 St (A S)", "Rockaway Park - Beach 116 St (A S)"],
    ["A53", "A54", "Van Siclen Av (C)", "Shepherd Av (C)"],
    ["A54", "A55", "Shepherd Av (C)", "Euclid Av (A C)"],
    ["A45", "A46", "Franklin Av (C)", "Nostrand Av (A C)"],
    ["A46", "A47", "Nostrand Av (A C)", "Kingston - Throop Avs (C)"],
    ["A47", "A48", "Kingston - Throop Avs (C)", "Utica Av (A C)"],
    ["A48", "A49", "Utica Av (A C)", "Ralph Av (C)"],
    ["A49", "A50", "Ralph Av (C)", "Rockaway Av (C)"],
    ["A50", "A51", "Rockaway Av (C)", "Broadway Jct (A C)"],
    ["A51", "A52", "Broadway Jct (A C)", "Liberty Av (C)"],
    ["A52", "A53", "Liberty Av (C)", "Van Siclen Av (C)"],
    ["A42", "A43", "Hoyt - Schermerhorn Sts (A C G)", "Lafayette Av (C)"],
    ["A43", "A44", "Lafayette Av (C)", "Clinton - Washington Avs (C)"],
    ["A44", "A45", "Clinton - Washington Avs (C)", "Franklin Av (C)"],
    ["A32", "A33", "W 4 St (A C E)", "Spring St (C E)"],
    ["A33", "A34", "Spring St (C E)", "Canal St (A C E)"],
    ["A20", "A21", "86 St (B C)", "81 St - Museum of Natural History (B C)"],
    ["A21", "A22", "81 St - Museum of Natural History (B C)", "72 St (B C)"],
    ["A22", "A24", "72 St (B C)", "59 St - Columbus Circle (A B C D)"],
    ["A24", "A25", "59 St - Columbus Circle (A B C D)", "50 St (C E)"],
    [
      "A25",
      "A27",
      "50 St (C E)",
      "42 St - Port Authority Bus Terminal (A C E)",
    ],
    ["A28", "A30", "34 St - Penn Station (A C E)", "23 St (C E)"],
    ["A30", "A31", "23 St (C E)", "14 St (A C E)"],
    ["A10", "A11", "163 St - Amsterdam Av (C)", "155 St (C)"],
    ["A11", "A12", "155 St (C)", "145 St (A C)"],
    ["A12", "A14", "145 St (A C)", "135 St (B C)"],
    ["A14", "A15", "135 St (B C)", "125 St (A B C D)"],
    ["A15", "A16", "125 St (A B C D)", "116 St (B C)"],
    ["A16", "A17", "116 St (B C)", "Cathedral Pkwy (110 St) (B C)"],
    ["A17", "A18", "Cathedral Pkwy (110 St) (B C)", "103 St (B C)"],
    ["A18", "A19", "103 St (B C)", "96 St (B C)"],
    ["A19", "A20", "96 St (B C)", "86 St (B C)"],
    ["A09", "A10", "168 St (A C)", "163 St - Amsterdam Av (C)"],
    ["A34", "E01", "Canal St (A C E)", "World Trade Center (E)"],
    ["F11", "F12", "Lexington Av/53 St (E M)", "5 Av/53 St (E M)"],
    ["F12", "D14", "5 Av/53 St (E M)", "7 Av (B D E)"],
    ["D14", "A25", "7 Av (B D E)", "50 St (C E)"],
    ["G21", "F09", "Queens Plaza (E M R)", "Court Sq (E M)"],
    ["F09", "F11", "Court Sq (E M)", "Lexington Av/53 St (E M)"],
    //["G14","G21","Jackson Hts - Roosevelt Av (E F M R)","Queens Plaza (E M R)"],  // 🚸 EXPRESS??
    //["G08","G14","Forest Hills - 71 Av (E F M R)","Jackson Hts - Roosevelt Av (E F M R)"],  // 🚸 EXPRESS??
    //["F06","G08","Kew Gardens - Union Tpke (E F)","Forest Hills - 71 Av (E F M R)",], // EXPRESS??
    [
      "G06",
      "G07",
      "Sutphin Blvd - Archer Av - JFK Airport (E J Z)",
      "Jamaica - Van Wyck (E)",
    ],
    //["G07", "F06", "Jamaica - Van Wyck (E)", "Kew Gardens - Union Tpke (E F)"], // EXPRESS??
    //["F01", "F03", "Jamaica - 179 St (F)", "Parsons Blvd (F)"], // ALTERNATE
    //["F03", "F06", "Parsons Blvd (F)", "Kew Gardens - Union Tpke (E F)"], // ALTERNATE
    [
      "G05",
      "G06",
      "Jamaica Center - Parsons/Archer (E J Z)",
      "Sutphin Blvd - Archer Av - JFK Airport (E J Z)",
    ],
    //["H19", "H04", "Unknown station Blue", "Broad Channel (A S)"], // UNKNOWN STATION
    //["A61", "A62", "Rockaway Blvd (A)", "Unknown station Blue"], //UNKNOWN STATION
    //["A62", "A63", "Unknown station Blue", "104 St (A)"], // UNKNOWN STATION
    ["G07", "F05", "Jamaica - Van Wyck (E)", "Briarwood - Van Wyck Blvd (E F)"],
    [
      "F05",
      "F06",
      "Briarwood - Van Wyck Blvd (E F)",
      "Kew Gardens - Union Tpke (E F)",
    ],
    ["F06", "F07", "Kew Gardens - Union Tpke (E F)", "75 Av (E F)"],
    ["F07", "G08", "75 Av (E F)", "Forest Hills - 71 Av (E F M R)"],
    //["A27","A31","42 St - Port Authority Bus Terminal (A C E)","14 St (A C E)",],  // EXPRESS??
    //["A28", "A29", "34 St - Penn Station (A C E)", "Unknown station Blue"], // UNKNOWN STATION
    //["A29", "A31", "Unknown station Blue", "14 St (A C E)"], UNKNOWN STATION
    //["A38", "A39", "Fulton St (A C)", "Unknown station Blue"], UNKNOWN STATION
    //["A39", "A40", "Unknown station Blue", "High St (A C)"], // UNKNOWN STATION
    //["A57", "A58", "Grant Av (A)", "Unknown station Blue"], // UNKNOWN STATION
    //["A58", "A59", "Unknown station Blue", "80 St (A)"], // UNKNOWN STATION
    //["A62", "H02", "Unknown station Blue", "Aqueduct - N Conduit Av (A)"], // UNKNOWN STATION
    //["H03", "H17", "Howard Beach - JFK Airport (A)", "Unknown station Blue"], // UNKNOWN STATION
    //["H17", "H04", "Unknown station Blue", "Broad Channel (A S)"], // UNKNOWN STATION
    //["H04", "H18", "Broad Channel (A S)", "Unknown station Blue"], // UNKNOWN STATION
    //["H18", "H05", "Unknown station Blue", "Unknown station Blue"], // UNKNOWN STATION
    //["H05", "H06", "Unknown station Blue", "Beach 67 St (A)"], // UNKNOWN STATION
    //["A32", "D21", "W 4 St (A C E)", "Broadway-Lafayette St (B D F M)"], // ALTERNATE
    // ["D21", "F14", "Broadway-Lafayette St (B D F M)", "2 Av (F)"], //ALTERNATE
    //["F14", "F15", "2 Av (F)", "Delancey St (F)"], // ALTERNATE
    //["G14","B04","Jackson Hts - Roosevelt Av (E F M R)","21 St - Queensbridge (F)",], // ALTERNATE
    //["B04", "B06", "21 St - Queensbridge (F)", "Roosevelt Island (F)"], // ALTERNATE
    //["B06", "B08", "Roosevelt Island (F)", "Lexington Av/63 St (F Q)"], // ALTERNATE
    //["B08", "B10", "Lexington Av/63 St (F Q)", "57 St (F)"], // ALTERNATE
    //["B10", "D15", "57 St (F)", "47-50 Sts - Rockefeller Ctr (B D F M)"], // ALTERNATE
    //["D15","D16","47-50 Sts - Rockefeller Ctr (B D F M)","42 St - Bryant Pk (B D F M)",], // ALTERNATE
    //["D16","D17","42 St - Bryant Pk (B D F M)","34 St - Herald Sq (B D F M)",], // ALTERNATE
    //["D17", "D18", "34 St - Herald Sq (B D F M)", "23 St (F M)"], // ALTERNATE
    //["D18", "D19", "23 St (F M)", "14 St (F M)"], // ALTERNATE
    //["D19", "D20", "14 St (F M)", "W 4 St (B D F M)"], // ALTERNATE
    //["D20", "D21", "W 4 St (B D F M)", "Broadway-Lafayette St (B D F M)"],
  ],
  Orange: [
    // ["D35", "D39", "Kings Hwy (B Q)", "Sheepshead Bay (B Q)"], // EXPRESS
    ["D39", "D40", "Sheepshead Bay (B Q)", "Brighton Beach (B Q)"],
    ["D25", "D26", "7 Av (B Q)", "Prospect Park (B Q)"],
    //["D26", "D28", "Prospect Park (B Q)", "Church Av (B Q)"], // EXPRESS
    //["D28", "D31", "Church Av (B Q)", "Newkirk Plaza (B Q)"], // EXPRESS
    //["D31", "D35", "Newkirk Plaza (B Q)", "Kings Hwy (B Q)"], // EXPRESS
    ["R30", "D24", "DeKalb Av (B Q R)", "Atlantic Av - Barclays Ctr (B Q)"],
    ["D24", "D25", "Atlantic Av - Barclays Ctr (B Q)", "7 Av (B Q)"],
    [
      "D16",
      "D17",
      "42 St - Bryant Pk (B D F M)",
      "34 St - Herald Sq (B D F M)",
    ],
    ["D17", "D20", "34 St - Herald Sq (B D F M)", "W 4 St (B D F M)"],
    ["D20", "D21", "W 4 St (B D F M)", "Broadway-Lafayette St (B D F M)"],
    ["D21", "D22", "Broadway-Lafayette St (B D F M)", "Grand St (B D)"],
    ["D22", "R30", "Grand St (B D)", "DeKalb Av (B Q R)"],
    ["A21", "A22", "81 St - Museum of Natural History (B C)", "72 St (B C)"],
    ["A22", "A24", "72 St (B C)", "59 St - Columbus Circle (A B C D)"],
    ["A24", "D14", "59 St - Columbus Circle (A B C D)", "7 Av (B D E)"],
    ["D14", "D15", "7 Av (B D E)", "47-50 Sts - Rockefeller Ctr (B D F M)"],
    [
      "D15",
      "D16",
      "47-50 Sts - Rockefeller Ctr (B D F M)",
      "42 St - Bryant Pk (B D F M)",
    ],
    ["A15", "A16", "125 St (A B C D)", "116 St (B C)"],
    ["A16", "A17", "116 St (B C)", "Cathedral Pkwy (110 St) (B C)"],
    ["A17", "A18", "Cathedral Pkwy (110 St) (B C)", "103 St (B C)"],
    ["A18", "A19", "103 St (B C)", "96 St (B C)"],
    ["A19", "A20", "96 St (B C)", "86 St (B C)"],
    ["A20", "A21", "86 St (B C)", "81 St - Museum of Natural History (B C)"],
    ["D13", "A14", "145 St (B D)", "135 St (B C)"],
    ["A14", "A15", "135 St (B C)", "125 St (A B C D)"],
    ["D03", "D04", "Bedford Park Blvd (B D)", "Kingsbridge Rd (B D)"],
    ["D04", "D05", "Kingsbridge Rd (B D)", "Fordham Rd (B D)"],
    ["D05", "D06", "Fordham Rd (B D)", "182-183 Sts (B D)"],
    ["D06", "D07", "182-183 Sts (B D)", "Tremont Av (B D)"],
    ["D07", "D08", "Tremont Av (B D)", "174-175 Sts (B D)"],
    ["D08", "D09", "174-175 Sts (B D)", "170 St (B D)"],
    ["D09", "D10", "170 St (B D)", "167 St (B D)"],
    ["D10", "D11", "167 St (B D)", "161 St - Yankee Stadium (B D)"],
    ["D11", "D12", "161 St - Yankee Stadium (B D)", "155 St (B D)"],
    ["D12", "D13", "155 St (B D)", "145 St (B D)"],
    ["B17", "B18", "71 St (D)", "79 St (D)"],
    ["B18", "B19", "79 St (D)", "18 Av (D)"],
    ["B19", "B20", "18 Av (D)", "20 Av (D)"],
    ["B20", "B21", "20 Av (D)", "Bay Pkwy (D)"],
    ["B21", "B22", "Bay Pkwy (D)", "25 Av (D)"],
    ["B22", "B23", "25 Av (D)", "Bay 50 St (D)"],
    ["B23", "D43", "Bay 50 St (D)", "Coney Island - Stillwell Av (D F N Q)"],
    ["B12", "B13", "9 Av (D)", "Fort Hamilton Pkwy (D)"],
    ["B13", "B14", "Fort Hamilton Pkwy (D)", "50 St (D)"],
    ["B14", "B15", "50 St (D)", "55 St (D)"],
    ["B15", "B16", "55 St (D)", "62 St (D)"],
    ["B16", "B17", "62 St (D)", "71 St (D)"],
    ["R36", "B12", "36 St (D N R)", "9 Av (D)"],
    //["D22", "R31", "Grand St (B D)", "Atlantic Av - Barclays Ctr (D N R)"], // EXPRESS
    //["R31", "R36", "Atlantic Av - Barclays Ctr (D N R)", "36 St (D N R)"], // EXPRESS
    //["D13", "A15", "145 St (B D)", "125 St (A B C D)"], // EXPRESS
    //["A15", "A24", "125 St (A B C D)", "59 St - Columbus Circle (A B C D)"], // EXPRESS
    ["D01", "D03", "Norwood - 205 St (D)", "Bedford Park Blvd (B D)"],
    [
      "D42",
      "D43",
      "W 8 St - NY Aquarium (F Q)",
      "Coney Island - Stillwell Av (D F N Q)",
    ],
    ["F32", "F33", "Bay Pkwy (F)", "Avenue N (F)"],
    ["F33", "F34", "Avenue N (F)", "Avenue P (F)"],
    ["F34", "F35", "Avenue P (F)", "Kings Hwy (F)"],
    ["F35", "F36", "Kings Hwy (F)", "Avenue U (F)"],
    ["F36", "F38", "Avenue U (F)", "Avenue X (F)"],
    ["F38", "F39", "Avenue X (F)", "Neptune Av (F)"],
    ["F39", "D42", "Neptune Av (F)", "W 8 St - NY Aquarium (F Q)"],
    ["F30", "F31", "18 Av (F)", "Avenue I (F)"],
    ["F31", "F32", "Avenue I (F)", "Bay Pkwy (F)"],
    ["F27", "F29", "Church Av (F G)", "Ditmas Av (F)"],
    ["F29", "F30", "Ditmas Av (F)", "18 Av (F)"],
    ["F23", "F24", "4 Av (F G)", "7 Av (F G)"],
    ["F24", "F25", "7 Av (F G)", "15 St - Prospect Park (F G)"],
    ["F25", "F26", "15 St - Prospect Park (F G)", "Fort Hamilton Pkwy (F G)"],
    ["F26", "F27", "Fort Hamilton Pkwy (F G)", "Church Av (F G)"],
    ["A41", "F20", "Jay St - MetroTech (A C F)", "Bergen St (F G)"],
    ["F20", "F21", "Bergen St (F G)", "Carroll St (F G)"],
    ["F21", "F22", "Carroll St (F G)", "Smith - 9 Sts (F G)"],
    ["F22", "F23", "Smith - 9 Sts (F G)", "4 Av (F G)"],
    ["F14", "F15", "2 Av (F)", "Delancey St (F)"],
    ["F15", "F16", "Delancey St (F)", "East Broadway (F)"],
    ["F16", "F18", "East Broadway (F)", "York St (F)"],
    ["F18", "A41", "York St (F)", "Jay St - MetroTech (A C F)"],
    ["D18", "D19", "23 St (F M)", "14 St (F M)"],
    ["D19", "D20", "14 St (F M)", "W 4 St (B D F M)"],
    ["D21", "F14", "Broadway-Lafayette St (B D F M)", "2 Av (F)"],
    ["B10", "D15", "57 St (F)", "47-50 Sts - Rockefeller Ctr (B D F M)"],
    ["D17", "D18", "34 St - Herald Sq (B D F M)", "23 St (F M)"],
    ["B04", "B06", "21 St - Queensbridge (F)", "Roosevelt Island (F)"],
    ["B06", "B08", "Roosevelt Island (F)", "Lexington Av/63 St (F Q)"],
    ["B08", "B10", "Lexington Av/63 St (F Q)", "57 St (F)"],
    //["G08","G14","Forest Hills - 71 Av (E F M R)","Jackson Hts - Roosevelt Av (E F M R)"], // EXPRESS
    //["G14","B04","Jackson Hts - Roosevelt Av (E F M R)","21 St - Queensbridge (F)"], // EXPRESS
    ["G20", "B04", "36 St (M R)", "21 St - Queensbridge (F)"], // Not a real interval, but where the tracks are
    ["F01", "F02", "Jamaica - 179 St (F)", "169 St (F)"],
    ["F02", "F03", "169 St (F)", "Parsons Blvd (F)"],
    ["F03", "F04", "Parsons Blvd (F)", "Sutphin Blvd (F)"],
    ["F04", "F05", "Sutphin Blvd (F)", "Briarwood - Van Wyck Blvd (E F)"],
    [
      "F05",
      "F06",
      "Briarwood - Van Wyck Blvd (E F)",
      "Kew Gardens - Union Tpke (E F)",
    ],
    ["F06", "F07", "Kew Gardens - Union Tpke (E F)", "75 Av (E F)"],
    ["F07", "G08", "75 Av (E F)", "Forest Hills - 71 Av (E F M R)"],
    ["M10", "M09", "Central Av (M)", "Knickerbocker Av (M)"],
    ["M09", "M08", "Knickerbocker Av (M)", "Myrtle - Wyckoff Avs (M)"],
    ["M08", "M06", "Myrtle - Wyckoff Avs (M)", "Seneca Av (M)"],
    ["M06", "M05", "Seneca Av (M)", "Forest Av (M)"],
    ["M05", "M04", "Forest Av (M)", "Fresh Pond Rd (M)"],
    ["M04", "M01", "Fresh Pond Rd (M)", "Middle Village - Metropolitan Av (M)"],
    ["M16", "M14", "Marcy Av (J M Z)", "Hewes St (J M)"],
    ["M14", "M13", "Hewes St (J M)", "Lorimer St (J M)"],
    ["M13", "M12", "Lorimer St (J M)", "Flushing Av (J M)"],
    ["M12", "M11", "Flushing Av (J M)", "Myrtle Av (J M Z)"],
    ["M11", "M10", "Myrtle Av (J M Z)", "Central Av (M)"],
    ["D21", "M18", "Broadway-Lafayette St (B D F M)", "Essex St (J M Z)"],
    ["M18", "M16", "Essex St (J M Z)", "Marcy Av (J M Z)"],
    ["F12", "D15", "5 Av/53 St (E M)", "47-50 Sts - Rockefeller Ctr (B D F M)"],
    ["G18", "G19", "46 St (M R)", "Steinway St (M R)"],
    ["G19", "G20", "Steinway St (M R)", "36 St (M R)"],
    ["G20", "G21", "36 St (M R)", "Queens Plaza (E M R)"],
    ["G21", "F09", "Queens Plaza (E M R)", "Court Sq (E M)"],
    ["F09", "F11", "Court Sq (E M)", "Lexington Av/53 St (E M)"],
    ["F11", "F12", "Lexington Av/53 St (E M)", "5 Av/53 St (E M)"],
    ["G12", "G13", "Grand Av - Newtown (M R)", "Elmhurst Av (M R)"],
    ["G13", "G14", "Elmhurst Av (M R)", "Jackson Hts - Roosevelt Av (E F M R)"],
    ["G14", "G15", "Jackson Hts - Roosevelt Av (E F M R)", "65 St (M R)"],
    ["G15", "G16", "65 St (M R)", "Northern Blvd (M R)"],
    ["G16", "G18", "Northern Blvd (M R)", "46 St (M R)"],
    ["G08", "G09", "Forest Hills - 71 Av (E F M R)", "67 Av (M R)"],
    ["G09", "G10", "67 Av (M R)", "63 Dr - Rego Park (M R)"],
    ["G10", "G11", "63 Dr - Rego Park (M R)", "Woodhaven Blvd (M R)"],
    ["G11", "G12", "Woodhaven Blvd (M R)", "Grand Av - Newtown (M R)"],
    //["B23", "B24", "Bay 50 St (D)", "Unknown station Orange"], // UNKNOWN STATION
    //["B24","D43","Unknown station Orange","Coney Island - Stillwell Av (D F N Q)"], // UNKNOWN STATION
    ["R30", "R31", "DeKalb Av (B Q R)", "Atlantic Av - Barclays Ctr (D N R)"],
    ["R31", "R32", "Atlantic Av - Barclays Ctr (D N R)", "Union St (R)"],
    ["R32", "R33", "Union St (R)", "9 St (R)"],
    ["R33", "R34", "9 St (R)", "Prospect Av (R)"],
    ["R34", "R35", "Prospect Av (R)", "25 St (R)"],
    ["R35", "R36", "25 St (R)", "36 St (D N R)"],
    //["G14","G21","Jackson Hts - Roosevelt Av (E F M R)","Queens Plaza (E M R)"], // EXPRESS
  ],
  LightGreen: [
    ["F24", "F25", "7 Av (F G)", "15 St - Prospect Park (F G)"],
    ["F25", "F26", "15 St - Prospect Park (F G)", "Fort Hamilton Pkwy (F G)"],
    ["F26", "F27", "Fort Hamilton Pkwy (F G)", "Church Av (F G)"],
    ["A42", "F20", "Hoyt - Schermerhorn Sts (A C G)", "Bergen St (F G)"],
    ["F20", "F21", "Bergen St (F G)", "Carroll St (F G)"],
    ["F21", "F22", "Carroll St (F G)", "Smith - 9 Sts (F G)"],
    ["F22", "F23", "Smith - 9 Sts (F G)", "4 Av (F G)"],
    ["F23", "F24", "4 Av (F G)", "7 Av (F G)"],
    ["G32", "G33", "Myrtle - Willoughby Avs (G)", "Bedford - Nostrand Avs (G)"],
    ["G33", "G34", "Bedford - Nostrand Avs (G)", "Classon Av (G)"],
    ["G34", "G35", "Classon Av (G)", "Clinton - Washington Avs (G)"],
    ["G35", "G36", "Clinton - Washington Avs (G)", "Fulton St (G)"],
    ["G36", "A42", "Fulton St (G)", "Hoyt - Schermerhorn Sts (A C G)"],
    ["G26", "G28", "Greenpoint Av (G)", "Nassau Av (G)"],
    ["G28", "G29", "Nassau Av (G)", "Metropolitan Av (G)"],
    ["G29", "G30", "Metropolitan Av (G)", "Broadway (G)"],
    ["G30", "G31", "Broadway (G)", "Flushing Av (G)"],
    ["G31", "G32", "Flushing Av (G)", "Myrtle - Willoughby Avs (G)"],
    ["G22", "G24", "Court Sq (G)", "21 St (G)"],
    ["G24", "G26", "21 St (G)", "Greenpoint Av (G)"],
  ],
  Brown: [
    ["M18", "M19", "Essex St (J M Z)", "Bowery (J Z)"],
    ["M19", "M20", "Bowery (J Z)", "Canal St (J Z)"],
    ["M20", "M21", "Canal St (J Z)", "Chambers St (J Z)"],
    ["M21", "M22", "Chambers St (J Z)", "Fulton St (J Z)"],
    ["M22", "M23", "Fulton St (J Z)", "Broad St (J Z)"],
    ["J31", "M11", "Kosciuszko St (J)", "Myrtle Av (J M Z)"],
    ["M11", "M12", "Myrtle Av (J M Z)", "Flushing Av (J M)"],
    ["M12", "M13", "Flushing Av (J M)", "Lorimer St (J M)"],
    ["M13", "M14", "Lorimer St (J M)", "Hewes St (J M)"],
    ["M14", "M16", "Hewes St (J M)", "Marcy Av (J M Z)"],
    ["M16", "M18", "Marcy Av (J M Z)", "Essex St (J M Z)"],
    ["J27", "J28", "Broadway Jct (J Z)", "Chauncey St (J Z)"],
    ["J28", "J29", "Chauncey St (J Z)", "Halsey St (J)"],
    ["J29", "J30", "Halsey St (J)", "Gates Av (J Z)"],
    ["J30", "J31", "Gates Av (J Z)", "Kosciuszko St (J)"],
    ["J21", "J22", "Norwood Av (J Z)", "Cleveland St (J)"],
    ["J22", "J23", "Cleveland St (J)", "Van Siclen Av (J Z)"],
    ["J23", "J24", "Van Siclen Av (J Z)", "Alabama Av (J)"],
    ["J24", "J27", "Alabama Av (J)", "Broadway Jct (J Z)"],
    ["J15", "J16", "Woodhaven Blvd (J Z)", "85 St - Forest Pkwy (J)"],
    ["J16", "J17", "85 St - Forest Pkwy (J)", "75 St (J Z)"],
    ["J17", "J19", "75 St (J Z)", "Cypress Hills (J)"],
    ["J19", "J20", "Cypress Hills (J)", "Crescent St (J Z)"],
    ["J20", "J21", "Crescent St (J Z)", "Norwood Av (J Z)"],
    ["J12", "J13", "121 St (J Z)", "111 St (J)"],
    ["J13", "J14", "111 St (J)", "104 St (J Z)"],
    ["J14", "J15", "104 St (J Z)", "Woodhaven Blvd (J Z)"],
    [
      "G05",
      "G06",
      "Jamaica Center - Parsons/Archer (E J Z)",
      "Sutphin Blvd - Archer Av - JFK Airport (E J Z)",
    ],
    [
      "G06",
      "J12",
      "Sutphin Blvd - Archer Av - JFK Airport (E J Z)",
      "121 St (J Z)",
    ],
  ],
  Yellow: [
    ["N10", "D43", "86 St (N)", "Coney Island - Stillwell Av (D F N Q)"],
    ["N02", "N03", "8 Av (N)", "Fort Hamilton Pkwy (N)"],
    ["N03", "N04", "Fort Hamilton Pkwy (N)", "New Utrecht Av (N)"],
    ["N04", "N05", "New Utrecht Av (N)", "18 Av (N)"],
    ["N05", "N06", "18 Av (N)", "20 Av (N)"],
    ["N06", "N07", "20 Av (N)", "Bay Pkwy (N)"],
    ["N07", "N08", "Bay Pkwy (N)", "Kings Hwy (N)"],
    ["N08", "N09", "Kings Hwy (N)", "Avenue U (N)"],
    ["N09", "N10", "Avenue U (N)", "86 St (N)"],
    //["R36", "R41", "36 St (D N R)", "59 St (N R)"], // EXPRESS
    ["R41", "N02", "59 St (N R)", "8 Av (N)"],
    ["R26", "R27", "Rector St (R W)", "Whitehall St (R W)"],
    //["Q01", "R31", "Canal St (N Q)", "Atlantic Av - Barclays Ctr (D N R)"], // EXPRESS
    //["R31", "R36", "Atlantic Av - Barclays Ctr (D N R)", "36 St (D N R)"], // EXPRESS
    ["R21", "R22", "8 St - NYU (R W)", "Prince St (R W)"],
    ["R22", "R23", "Prince St (R W)", "Canal St (R W)"],
    ["R23", "R24", "Canal St (R W)", "City Hall (R W)"],
    ["R24", "R25", "City Hall (R W)", "Cortlandt St (R W)"],
    ["R25", "R26", "Cortlandt St (R W)", "Rector St (R W)"],
    ["R16", "R17", "Times Sq - 42 St (N Q R W)", "34 St - Herald Sq (N Q R W)"],
    //["R17", "R20", "34 St - Herald Sq (N Q R W)", "14 St - Union Sq (N Q R W)"], // EXPRESS
    //["R20", "Q01", "14 St - Union Sq (N Q R W)", "Canal St (N Q)"], // EXPRESS
    ["R13", "R14", "5 Av/59 St (N W R)", "57 St - 7 Av (N Q R W)"],
    ["R14", "R15", "57 St - 7 Av (N Q R W)", "49 St (N R W)"],
    ["R15", "R16", "49 St (N R W)", "Times Sq - 42 St (N Q R W)"],
    ["R17", "R18", "34 St - Herald Sq (N Q R W)", "28 St (R W)"],
    ["R18", "R19", "28 St (R W)", "23 St (R W)"],
    ["R19", "R20", "23 St (R W)", "14 St - Union Sq (N Q R W)"],
    ["R20", "R21", "14 St - Union Sq (N Q R W)", "8 St - NYU (R W)"],
    ["R11", "R13", "Lexington Av/59 St (N W R)", "5 Av/59 St (N W R)"],
    ["R08", "R09", "39 Av (N W)", "Queensboro Plaza (N W)"],
    //["R09", "R60", "Queensboro Plaza (N W)", "Unknown station Yellow"], // UNKNOWN STATION
    // ["R60", "R11", "Unknown station Yellow", "Lexington Av/59 St (N W R)"], // UNKNOWN STATION
    ["R09", "R11", "Queensboro Plaza (N W)", "Lexington Av/59 St (N W R)"], // COMBINED FROM ABOVE
    ["R01", "R03", "Astoria - Ditmars Blvd (N W)", "Astoria Blvd (N W)"],
    ["R03", "R04", "Astoria Blvd (N W)", "30 Av (N W)"],
    ["R04", "R05", "30 Av (N W)", "Broadway (N W)"],
    ["R05", "R06", "Broadway (N W)", "36 Av (N W)"],
    ["R06", "R08", "36 Av (N W)", "39 Av (N W)"],
    ["Q05", "Q04", "96 St (Q)", "86 St (Q)"],
    ["Q04", "Q03", "86 St (Q)", "72 St (Q)"],
    ["Q03", "B08", "72 St (Q)", "Lexington Av/63 St (F Q)"],
    ["B08", "R14", "Lexington Av/63 St (F Q)", "57 St - 7 Av (N Q R W)"],
    //["R14", "R16", "57 St - 7 Av (N Q R W)", "Times Sq - 42 St (N Q R W)"], // EXPRESS
    [
      "D42",
      "D43",
      "W 8 St - NY Aquarium (F Q)",
      "Coney Island - Stillwell Av (D F N Q)",
    ],
    ["D40", "D41", "Brighton Beach (B Q)", "Ocean Pkwy (Q)"],
    ["D41", "D42", "Ocean Pkwy (Q)", "W 8 St - NY Aquarium (F Q)"],
    ["D31", "D32", "Newkirk Plaza (B Q)", "Avenue H (B Q)"],
    ["D34", "D35", "Avenue M (B Q)", "Kings Hwy (B Q)"],
    ["D35", "D37", "Kings Hwy (B Q)", "Avenue U (B Q)"],
    ["D37", "D38", "Avenue U (B Q)", "Neck Rd (B Q)"],
    ["D38", "D39", "Neck Rd (B Q)", "Sheepshead Bay (B Q)"],
    ["D39", "D40", "Sheepshead Bay (B Q)", "Brighton Beach (B Q)"],
    ["D27", "D28", "Parkside Av (B Q)", "Church Av (B Q)"],
    ["D28", "D29", "Church Av (B Q)", "Beverley Rd (B Q)"],
    ["D29", "D30", "Beverley Rd (B Q)", "Cortelyou Rd (B Q)"],
    ["D30", "D31", "Cortelyou Rd (B Q)", "Newkirk Plaza (B Q)"],
    ["D32", "D33", "Avenue H (B Q)", "Avenue J (B Q)"],
    ["D33", "D34", "Avenue J (B Q)", "Avenue M (B Q)"],
    ["R30", "D24", "DeKalb Av (B Q R)", "Atlantic Av - Barclays Ctr (B Q)"],
    ["D24", "D25", "Atlantic Av - Barclays Ctr (B Q)", "7 Av (B Q)"],
    ["D25", "D26", "7 Av (B Q)", "Prospect Park (B Q)"],
    ["D26", "D27", "Prospect Park (B Q)", "Parkside Av (B Q)"],
    ["Q01", "R30", "Canal St (N Q)", "DeKalb Av (B Q R)"],
    ["R42", "R43", "Bay Ridge Av (R)", "77 St (R)"],
    ["R43", "R44", "77 St (R)", "86 St (R)"],
    ["R44", "R45", "86 St (R)", "Bay Ridge - 95 St (R)"],
    ["R34", "R35", "Prospect Av (R)", "25 St (R)"],
    ["R35", "R36", "25 St (R)", "36 St (D N R)"],
    ["R36", "R39", "36 St (D N R)", "45 St (R)"],
    ["R39", "R40", "45 St (R)", "53 St (R)"],
    ["R40", "R41", "53 St (R)", "59 St (N R)"],
    //["R41", "R42", "59 St (N R)", "Bay Ridge Av (R)"], // EXPRESS
    ["R31", "R32", "Atlantic Av - Barclays Ctr (D N R)", "Union St (R)"],
    ["R32", "R33", "Union St (R)", "9 St (R)"],
    ["R33", "R34", "9 St (R)", "Prospect Av (R)"],
    //["R27", "R65", "Whitehall St (R W)", "Unknown station Yellow"], // UNKNOWN STATION
    //["R65", "R28", "Unknown station Yellow", "Court St (R)"], // UNKNOWN STATION
    ["R27", "R28", "Whitehall St (R W)", "Court St (R)"], // COMBINED FROM ABOVE
    ["R28", "R29", "Court St (R)", "Jay St - MetroTech (R)"],
    ["R29", "R30", "Jay St - MetroTech (R)", "DeKalb Av (B Q R)"],
    ["R30", "R31", "DeKalb Av (B Q R)", "Atlantic Av - Barclays Ctr (D N R)"],
    //["G21", "R60", "Queens Plaza (E M R)", "Unknown station Yellow"], // UNKNOWN STATION
    ["G21", "R11", "Queens Plaza (E M R)", "Lexington Av/59 St (N W R)"], // MISSING
    ["G16", "G18", "Northern Blvd (M R)", "46 St (M R)"],
    ["G18", "G19", "46 St (M R)", "Steinway St (M R)"],
    ["G19", "G20", "Steinway St (M R)", "36 St (M R)"],
    ["G20", "G21", "36 St (M R)", "Queens Plaza (E M R)"],
    ["G13", "G14", "Elmhurst Av (M R)", "Jackson Hts - Roosevelt Av (E F M R)"],
    ["G14", "G15", "Jackson Hts - Roosevelt Av (E F M R)", "65 St (M R)"],
    ["G15", "G16", "65 St (M R)", "Northern Blvd (M R)"],
    ["G08", "G09", "Forest Hills - 71 Av (E F M R)", "67 Av (M R)"],
    ["G09", "G10", "67 Av (M R)", "63 Dr - Rego Park (M R)"],
    ["G10", "G11", "63 Dr - Rego Park (M R)", "Woodhaven Blvd (M R)"],
    ["G11", "G12", "Woodhaven Blvd (M R)", "Grand Av - Newtown (M R)"],
    ["G12", "G13", "Grand Av - Newtown (M R)", "Elmhurst Av (M R)"],
    ["R22", "Q01", "Prince St (R W)", "Canal St (N Q)"],
  ],
  LightGrey: [
    ["L26", "L27", "Livonia Av (L)", "New Lots Av (L)"],
    ["L27", "L28", "New Lots Av (L)", "E 105 St (L)"],
    ["L28", "L29", "E 105 St (L)", "Canarsie - Rockaway Pkwy (L)"],
    ["L21", "L22", "Bushwick Av - Aberdeen St (L)", "Broadway Jct (L)"],
    ["L22", "L24", "Broadway Jct (L)", "Atlantic Av (L)"],
    ["L24", "L25", "Atlantic Av (L)", "Sutter Av (L)"],
    ["L25", "L26", "Sutter Av (L)", "Livonia Av (L)"],
    ["L16", "L17", "DeKalb Av (L)", "Myrtle - Wyckoff Avs (L)"],
    ["L17", "L19", "Myrtle - Wyckoff Avs (L)", "Halsey St (L)"],
    ["L19", "L20", "Halsey St (L)", "Wilson Av (L)"],
    ["L20", "L21", "Wilson Av (L)", "Bushwick Av - Aberdeen St (L)"],
    ["L14", "L15", "Morgan Av (L)", "Jefferson St (L)"],
    ["L15", "L16", "Jefferson St (L)", "DeKalb Av (L)"],
    ["L10", "L11", "Lorimer St (L)", "Graham Av (L)"],
    ["L11", "L12", "Graham Av (L)", "Grand St (L)"],
    ["L12", "L13", "Grand St (L)", "Montrose Av (L)"],
    ["L13", "L14", "Montrose Av (L)", "Morgan Av (L)"],
    ["L06", "L08", "1 Av (L)", "Bedford Av (L)"],
    ["L08", "L10", "Bedford Av (L)", "Lorimer St (L)"],
    ["L01", "L02", "8 Av (L)", "6 Av (L)"],
    ["L02", "L03", "6 Av (L)", "Union Sq - 14 St (L)"],
    ["L03", "L05", "Union Sq - 14 St (L)", "3 Av (L)"],
    ["L05", "L06", "3 Av (L)", "1 Av (L)"],
  ],
  Red: [
    ["139", "142", "Rector St (1)", "South Ferry (1)"],
    ["136", "137", "Franklin St (1)", "Chambers St (1 2 3)"],
    ["137", "138", "Chambers St (1 2 3)", "WTC Cortlandt (1)"],
    ["138", "139", "WTC Cortlandt (1)", "Rector St (1)"],
    ["132", "133", "14 St (1 2 3)", "Christopher St - Sheridan Sq (1)"],
    ["133", "134", "Christopher St - Sheridan Sq (1)", "Houston St (1)"],
    ["134", "135", "Houston St (1)", "Canal St (1)"],
    ["135", "136", "Canal St (1)", "Franklin St (1)"],
    ["127", "128", "Times Sq - 42 St (1 2 3)", "34 St - Penn Station (1 2 3)"],
    ["128", "129", "34 St - Penn Station (1 2 3)", "28 St (1)"],
    ["129", "130", "28 St (1)", "23 St (1)"],
    ["130", "131", "23 St (1)", "18 St (1)"],
    ["131", "132", "18 St (1)", "14 St (1 2 3)"],
    ["125", "126", "59 St - Columbus Circle (1)", "50 St (1)"],
    ["126", "127", "50 St (1)", "Times Sq - 42 St (1 2 3)"],
    ["122", "123", "79 St (1)", "72 St (1 2 3)"],
    ["123", "124", "72 St (1 2 3)", "66 St - Lincoln Center (1)"],
    ["124", "125", "66 St - Lincoln Center (1)", "59 St - Columbus Circle (1)"],
    ["119", "120", "103 St (1)", "96 St (1 2 3)"],
    ["120", "121", "96 St (1 2 3)", "86 St (1)"],
    ["121", "122", "86 St (1)", "79 St (1)"],
    ["115", "116", "137 St - City College (1)", "125 St (1)"],
    ["116", "117", "125 St (1)", "116 St - Columbia University (1)"],
    ["117", "118", "116 St - Columbia University (1)", "Cathedral Pkwy (1)"],
    ["118", "119", "Cathedral Pkwy (1)", "103 St (1)"],
    ["113", "114", "157 St (1)", "145 St (1)"],
    ["114", "115", "145 St (1)", "137 St - City College (1)"],
    ["110", "111", "191 St (1)", "181 St (1)"],
    ["111", "112", "181 St (1)", "168 St - Washington Hts (1)"],
    ["112", "113", "168 St - Washington Hts (1)", "157 St (1)"],
    ["106", "107", "Marble Hill - 225 St (1)", "215 St (1)"],
    ["107", "108", "215 St (1)", "207 St (1)"],
    ["108", "109", "207 St (1)", "Dyckman St (1)"],
    ["109", "110", "Dyckman St (1)", "191 St (1)"],
    ["101", "103", "Van Cortlandt Park - 242 St (1)", "238 St (1)"],
    ["103", "104", "238 St (1)", "231 St (1)"],
    ["104", "106", "231 St (1)", "Marble Hill - 225 St (1)"],
    ["246", "247", "Newkirk Av (2 5)", "Flatbush Av - Brooklyn College (2 5)"],
    ["235", "236", "Atlantic Av - Barclays Ctr (2 3)", "Bergen St (2 3)"],
    ["236", "237", "Bergen St (2 3)", "Grand Army Plaza (2 3)"],
    [
      "237",
      "238",
      "Grand Army Plaza (2 3)",
      "Eastern Pkwy - Brooklyn Museum (2 3)",
    ],
    [
      "238",
      "239",
      "Eastern Pkwy - Brooklyn Museum (2 3)",
      "Franklin Av (2 3 4 5)",
    ],
    ["239", "241", "Franklin Av (2 3 4 5)", "President St (2 5)"],
    ["241", "242", "President St (2 5)", "Sterling St (2 5)"],
    ["242", "243", "Sterling St (2 5)", "Winthrop St (2 5)"],
    ["243", "244", "Winthrop St (2 5)", "Church Av (2 5)"],
    ["244", "245", "Church Av (2 5)", "Beverly Rd (2 5)"],
    ["245", "246", "Beverly Rd (2 5)", "Newkirk Av (2 5)"],
    ["231", "232", "Clark St (2 3)", "Borough Hall (2 3)"], // NO GREEN
    ["232", "233", "Borough Hall (2 3)", "Hoyt St (2 3)"], // NO GREEN
    ["233", "234", "Hoyt St (2 3)", "Nevins St (2 3 4 5)"], // NO GREEN
    ["234", "235", "Nevins St (2 3 4 5)", "Atlantic Av - Barclays Ctr (2 3)"],
    ["137", "228", "Chambers St (1 2 3)", "Park Pl (2 3)"], // NO GREEN
    ["228", "229", "Park Pl (2 3)", "Fulton St (2 3)"], // NO GREEN
    ["229", "230", "Fulton St (2 3)", "Wall St (2 3)"], // NO GREEN
    ["230", "231", "Wall St (2 3)", "Clark St (2 3)"], // NO GREEN
    // ["120", "123", "96 St (1 2 3)", "72 St (1 2 3)"], // EXPRESS
    // ["128", "132", "34 St - Penn Station (1 2 3)", "14 St (1 2 3)"], // EXPRESS
    // ["132", "137", "14 St (1 2 3)", "Chambers St (1 2 3)"], // EXPRESS
    ["226", "227", "116 St (2 3)", "Central Park North (110 St) (2 3)"], // NO GREEN
    ["227", "120", "Central Park North (110 St) (2 3)", "96 St (1 2 3)"], // NO GREEN
    ["224", "225", "135 St (2 3)", "125 St (2 3)"], // NO GREEN
    ["225", "226", "125 St (2 3)", "116 St (2 3)"], // NO GREEN
    ["217", "218", "Simpson St (2 5)", "Intervale Av (2 5)"],
    ["218", "219", "Intervale Av (2 5)", "Prospect Av (2 5)"],
    ["219", "220", "Prospect Av (2 5)", "Jackson Av (2 5)"],
    ["220", "221", "Jackson Av (2 5)", "3 Av - 149 St (2 5)"],
    ["221", "222", "3 Av - 149 St (2 5)", "149 St - Grand Concourse (2 5)"],
    ["222", "224", "149 St - Grand Concourse (2 5)", "135 St (2 3)"], // NO GREEN
    ["215", "216", "174 St (2 5)", "Freeman St (2 5)"],
    ["216", "217", "Freeman St (2 5)", "Simpson St (2 5)"],
    ["212", "213", "Bronx Park East (2 5)", "E 180 St (2 5)"], // GREEN DASHED
    ["213", "214", "E 180 St (2 5)", "West Farms Sq - E Tremont Av (2 5)"],
    ["214", "215", "West Farms Sq - E Tremont Av (2 5)", "174 St (2 5)"],
    ["207", "208", "219 St (2 5)", "Gun Hill Rd (2 5)"], // GREEN DASHED
    ["208", "209", "Gun Hill Rd (2 5)", "Burke Av (2 5)"], // GREEN DASHED
    ["209", "210", "Burke Av (2 5)", "Allerton Av (2 5)"], // GREEN DASHED
    ["210", "211", "Allerton Av (2 5)", "Pelham Pkwy (2 5)"], // GREEN DASHED
    ["211", "212", "Pelham Pkwy (2 5)", "Bronx Park East (2 5)"], // GREEN DASHED
    ["201", "204", "Wakefield - 241 St (2)", "Nereid Av (2 5)"], // NO GREEN
    ["204", "205", "Nereid Av (2 5)", "233 St (2 5)"], // GREEN DASHED
    ["205", "206", "233 St (2 5)", "225 St (2 5)"], // GREEN DASHED
    ["206", "207", "225 St (2 5)", "219 St (2 5)"], // GREEN DASHED
    ["254", "255", "Junius St (3)", "Pennsylvania Av (3)"], // NO GREEN
    ["255", "256", "Pennsylvania Av (3)", "Van Siclen Av (3)"], // NO GREEN
    ["256", "257", "Van Siclen Av (3)", "New Lots Av (3)"], // NO GREEN
    ["253", "254", "Rockaway Av (3)", "Junius St (3)"], // NO GREEN
    ["239", "248", "Franklin Av (2 3 4 5)", "Nostrand Av (3)"],
    ["248", "249", "Nostrand Av (3)", "Kingston Av (3)"],
    ["249", "250", "Kingston Av (3)", "Crown Hts - Utica Av (3 4)"],
    ["250", "251", "Crown Hts - Utica Av (3 4)", "Sutter Av - Rutland Rd (3)"], // NO GREEN
    ["251", "252", "Sutter Av - Rutland Rd (3)", "Saratoga Av (3)"], // NO GREEN
    ["252", "253", "Saratoga Av (3)", "Rockaway Av (3)"], // NO GREEN
    ["301", "302", "Harlem - 148 St (3)", "145 St (3)"], // NO GREEN
    ["302", "224", "145 St (3)", "135 St (2 3)"], // NO GREEN
    ["423", "234", "Borough Hall (4 5)", "Nevins St (2 3 4 5)"],
    // ["235", "239", "Atlantic Av - Barclays Ctr (2 3)", "Franklin Av (2 3 4 5)"], // EXPRESS
    // ["239", "250", "Franklin Av (2 3 4 5)", "Crown Hts - Utica Av (3 4)"], // EXPRESS
  ],
  Green: [
    ["246", "247", "Newkirk Av (2 5)", "Flatbush Av - Brooklyn College (2 5)"],
    ["235", "236", "Atlantic Av - Barclays Ctr (2 3)", "Bergen St (2 3)"],
    ["236", "237", "Bergen St (2 3)", "Grand Army Plaza (2 3)"],
    [
      "237",
      "238",
      "Grand Army Plaza (2 3)",
      "Eastern Pkwy - Brooklyn Museum (2 3)",
    ],
    [
      "238",
      "239",
      "Eastern Pkwy - Brooklyn Museum (2 3)",
      "Franklin Av (2 3 4 5)",
    ],
    ["239", "241", "Franklin Av (2 3 4 5)", "President St (2 5)"],
    ["241", "242", "President St (2 5)", "Sterling St (2 5)"],
    ["242", "243", "Sterling St (2 5)", "Winthrop St (2 5)"],
    ["243", "244", "Winthrop St (2 5)", "Church Av (2 5)"],
    ["244", "245", "Church Av (2 5)", "Beverly Rd (2 5)"],
    ["245", "246", "Beverly Rd (2 5)", "Newkirk Av (2 5)"],
    ["234", "235", "Nevins St (2 3 4 5)", "Atlantic Av - Barclays Ctr (2 3)"],
    ["217", "218", "Simpson St (2 5)", "Intervale Av (2 5)"],
    ["218", "219", "Intervale Av (2 5)", "Prospect Av (2 5)"],
    ["219", "220", "Prospect Av (2 5)", "Jackson Av (2 5)"],
    ["220", "221", "Jackson Av (2 5)", "3 Av - 149 St (2 5)"],
    ["221", "222", "3 Av - 149 St (2 5)", "149 St - Grand Concourse (2 5)"],
    ["215", "216", "174 St (2 5)", "Freeman St (2 5)"],
    ["216", "217", "Freeman St (2 5)", "Simpson St (2 5)"],
    ["213", "214", "E 180 St (2 5)", "West Farms Sq - E Tremont Av (2 5)"],
    ["214", "215", "West Farms Sq - E Tremont Av (2 5)", "174 St (2 5)"],
    // ["207", "208", "219 St (2 5)", "Gun Hill Rd (2 5)"], // GREEN DASHED
    // ["208", "209", "Gun Hill Rd (2 5)", "Burke Av (2 5)"], // GREEN DASHED
    // ["209", "210", "Burke Av (2 5)", "Allerton Av (2 5)"], // GREEN DASHED
    // ["210", "211", "Allerton Av (2 5)", "Pelham Pkwy (2 5)"], // GREEN DASHED
    // ["211", "212", "Pelham Pkwy (2 5)", "Bronx Park East (2 5)"], // GREEN DASHED
    // ["204", "205", "Nereid Av (2 5)", "233 St (2 5)"], // GREEN DASHED
    // ["205", "206", "233 St (2 5)", "225 St (2 5)"], // GREEN DASHED
    // ["206", "207", "225 St (2 5)", "219 St (2 5)"], // GREEN DASHED
    ["239", "248", "Franklin Av (2 3 4 5)", "Nostrand Av (3)"],
    ["248", "249", "Nostrand Av (3)", "Kingston Av (3)"],
    ["249", "250", "Kingston Av (3)", "Crown Hts - Utica Av (3 4)"],
    ["423", "234", "Borough Hall (4 5)", "Nevins St (2 3 4 5)"],
    ["420", "423", "Bowling Green (4 5)", "Borough Hall (4 5)"],
    ["640", "418", "Brooklyn Bridge - City Hall (4 5 6)", "Fulton St (4 5)"],
    ["418", "419", "Fulton St (4 5)", "Wall St (4 5)"],
    ["419", "420", "Wall St (4 5)", "Bowling Green (4 5)"],
    // ["629", "631", "59 St (4 5 6)", "Grand Central - 42 St (4 5 6)"], // EXPRESS
    // ["631", "635", "Grand Central - 42 St (4 5 6)", "14 St - Union Sq (4 5 6)"], // EXPRESS
    // ["635","640","14 St - Union Sq (4 5 6)","Brooklyn Bridge - City Hall (4 5 6)"], // EXPRESS
    // ["621", "626", "125 St (4 5 6)", "86 St (4 5 6)"], // EXPRESS
    // ["626", "629", "86 St (4 5 6)", "59 St (4 5 6)"], // EXPRESS
    [
      "415",
      "416",
      "149 St - Grand Concourse (4)",
      "138 St - Grand Concourse (4 5)",
    ],
    ["416", "621", "138 St - Grand Concourse (4 5)", "125 St (4 5 6)"],
    ["412", "413", "170 St (4)", "167 St (4)"],
    ["413", "414", "167 St (4)", "161 St - Yankee Stadium (4)"],
    [
      "414",
      "415",
      "161 St - Yankee Stadium (4)",
      "149 St - Grand Concourse (4)",
    ],
    ["409", "410", "Burnside Av (4)", "176 St (4)"],
    ["410", "411", "176 St (4)", "Mt Eden Av (4)"],
    ["411", "412", "Mt Eden Av (4)", "170 St (4)"],
    [
      "402",
      "405",
      "Mosholu Pkwy (4)",
      "Bedford Park Blvd - Lehman College (4)",
    ],
    [
      "405",
      "406",
      "Bedford Park Blvd - Lehman College (4)",
      "Kingsbridge Rd (4)",
    ],
    ["406", "407", "Kingsbridge Rd (4)", "Fordham Rd (4)"],
    ["407", "408", "Fordham Rd (4)", "183 St (4)"],
    ["408", "409", "183 St (4)", "Burnside Av (4)"],
    ["401", "402", "Woodlawn (4)", "Mosholu Pkwy (4)"],
    [
      "222",
      "416",
      "149 St - Grand Concourse (2 5)",
      "138 St - Grand Concourse (4 5)",
    ],
    ["504", "505", "Pelham Pkwy (5)", "Morris Park (5)"],
    ["505", "213", "Morris Park (5)", "E 180 St (2 5)"],
    ["502", "503", "Baychester Av (5)", "Gun Hill Rd (5)"],
    ["503", "504", "Gun Hill Rd (5)", "Pelham Pkwy (5)"],
    ["501", "502", "Eastchester - Dyre Av (5)", "Baychester Av (5)"],
    ["638", "639", "Spring St (6)", "Canal St (6)"],
    ["639", "640", "Canal St (6)", "Brooklyn Bridge - City Hall (4 5 6)"],
    ["635", "636", "14 St - Union Sq (4 5 6)", "Astor Pl (6)"],
    ["636", "637", "Astor Pl (6)", "Bleecker St (6)"],
    ["637", "638", "Bleecker St (6)", "Spring St (6)"],
    ["630", "631", "51 St (6)", "Grand Central - 42 St (4 5 6)"],
    ["631", "632", "Grand Central - 42 St (4 5 6)", "33 St (6)"],
    ["632", "633", "33 St (6)", "28 St (6)"],
    ["633", "634", "28 St (6)", "23 St (6)"],
    ["634", "635", "23 St (6)", "14 St - Union Sq (4 5 6)"],
    ["627", "628", "77 St (6)", "68 St - Hunter College (6)"],
    ["628", "629", "68 St - Hunter College (6)", "59 St (4 5 6)"],
    ["629", "630", "59 St (4 5 6)", "51 St (6)"],
    ["622", "623", "116 St (6)", "110 St (6)"],
    ["623", "624", "110 St (6)", "103 St (6)"],
    ["624", "625", "103 St (6)", "96 St (6)"],
    ["625", "626", "96 St (6)", "86 St (4 5 6)"],
    ["626", "627", "86 St (4 5 6)", "77 St (6)"],
    ["619", "621", "3 Av - 138 St (6)", "125 St (4 5 6)"],
    ["621", "622", "125 St (4 5 6)", "116 St (6)"],
    ["616", "617", "E 143 St - St Mary's St (6)", "Cypress Av (6)"],
    ["617", "618", "Cypress Av (6)", "Brook Av (6)"],
    ["618", "619", "Brook Av (6)", "3 Av - 138 St (6)"],
    ["610", "611", "Morrison Av- Sound View (6)", "Elder Av (6)"],
    ["611", "612", "Elder Av (6)", "Whitlock Av (6)"],
    ["612", "613", "Whitlock Av (6)", "Hunts Point Av (6)"],
    ["613", "614", "Hunts Point Av (6)", "Longwood Av (6)"],
    ["614", "615", "Longwood Av (6)", "E 149 St (6)"],
    ["615", "616", "E 149 St (6)", "E 143 St - St Mary's St (6)"],
    ["607", "608", "Castle Hill Av (6)", "Parkchester (6)"],
    ["608", "609", "Parkchester (6)", "St Lawrence Av (6)"],
    ["609", "610", "St Lawrence Av (6)", "Morrison Av- Sound View (6)"],
    ["602", "603", "Buhre Av (6)", "Middletown Rd (6)"],
    ["603", "604", "Middletown Rd (6)", "Westchester Sq - E Tremont Av (6)"],
    ["604", "606", "Westchester Sq - E Tremont Av (6)", "Zerega Av (6)"],
    ["606", "607", "Zerega Av (6)", "Castle Hill Av (6)"],
    ["601", "602", "Pelham Bay Park (6)", "Buhre Av (6)"],
    // ["613", "619", "Hunts Point Av (6)", "3 Av - 138 St (6)"], // EXPRESS
  ],
  DarkGrey: [
    ["902", "901", "Times Sq - 42 St (S)", "Grand Central - 42 St (S)"],
    ["S01", "S03", "Franklin Av (S)", "Park Pl (S)"],
    ["S03", "S04", "Park Pl (S)", "Botanic Garden (S)"],
    ["S04", "D26", "Botanic Garden (S)", "Prospect Park (B Q)"],
    ["H04", "H12", "Broad Channel (A S)", "Beach 90 St (A S)"],
    ["H12", "H13", "Beach 90 St (A S)", "Beach 98 St (A S)"],
    ["H13", "H14", "Beach 98 St (A S)", "Beach 105 St (A S)"],
    ["H14", "H15", "Beach 105 St (A S)", "Rockaway Park - Beach 116 St (A S)"],
  ],
  Purple: [
    ["723", "724", "Grand Central - 42 St (7)", "5 Av (7)"],
    ["724", "725", "5 Av (7)", "Times Sq - 42 St (7)"],
    ["725", "726", "Times Sq - 42 St (7)", "34 St - 11 Av (7)"],
    ["720", "721", "Hunters Point Av (7)", "Vernon Blvd - Jackson Av (7)"],
    ["721", "723", "Vernon Blvd - Jackson Av (7)", "Grand Central - 42 St (7)"],
    ["718", "719", "Queensboro Plaza (7)", "Court Sq (7)"],
    ["719", "720", "Court Sq (7)", "Hunters Point Av (7)"],
    ["716", "718", "33 St (7)", "Queensboro Plaza (7)"],
    ["714", "715", "46 St (7)", "40 St (7)"],
    ["715", "716", "40 St (7)", "33 St (7)"],
    ["710", "711", "74 St - Broadway (7)", "69 St (7)"],
    ["711", "712", "69 St (7)", "Woodside - 61 St (7)"],
    ["712", "713", "Woodside - 61 St (7)", "52 St (7)"],
    ["713", "714", "52 St (7)", "46 St (7)"],
    ["706", "707", "103 St - Corona Plaza (7)", "Junction Blvd (7)"],
    ["707", "708", "Junction Blvd (7)", "90 St - Elmhurst Av (7)"],
    ["708", "709", "90 St - Elmhurst Av (7)", "82 St - Jackson Hts (7)"],
    ["709", "710", "82 St - Jackson Hts (7)", "74 St - Broadway (7)"],
    ["701", "702", "Flushing - Main St (7)", "Mets - Willets Point (7)"],
    ["702", "705", "Mets - Willets Point (7)", "111 St (7)"],
    ["705", "706", "111 St (7)", "103 St - Corona Plaza (7)"],
  ],
  SteelBlue: [
    ["S16", "S15", "Huguenot (SIR)", "Prince's Bay (SIR)"],
    ["S15", "S14", "Prince's Bay (SIR)", "Pleasant Plains (SIR)"],
    ["S14", "S13", "Pleasant Plains (SIR)", "Richmond Valley (SIR)"],
    ["S13", "S11", "Richmond Valley (SIR)", "Arthur Kill (SIR)"],
    ["S11", "S09", "Arthur Kill (SIR)", "Tottenville (SIR)"],
    ["S26", "S25", "Old Town (SIR)", "Dongan Hills (SIR)"],
    ["S25", "S24", "Dongan Hills (SIR)", "Jefferson Av (SIR)"],
    ["S24", "S23", "Jefferson Av (SIR)", "Grant City (SIR)"],
    ["S23", "S22", "Grant City (SIR)", "New Dorp (SIR)"],
    ["S22", "S21", "New Dorp (SIR)", "Oakwood Heights (SIR)"],
    ["S21", "S20", "Oakwood Heights (SIR)", "Bay Terrace (SIR)"],
    ["S20", "S19", "Bay Terrace (SIR)", "Great Kills (SIR)"],
    ["S19", "S18", "Great Kills (SIR)", "Eltingville (SIR)"],
    ["S18", "S17", "Eltingville (SIR)", "Annadale (SIR)"],
    ["S17", "S16", "Annadale (SIR)", "Huguenot (SIR)"],
    ["S31", "S30", "St George (SIR)", "Tompkinsville (SIR)"],
    ["S30", "S29", "Tompkinsville (SIR)", "Stapleton (SIR)"],
    ["S29", "S28", "Stapleton (SIR)", "Clifton (SIR)"],
    ["S28", "S27", "Clifton (SIR)", "Grasmere (SIR)"],
    ["S27", "S26", "Grasmere (SIR)", "Old Town (SIR)"],
  ],
};

export default lineGroupIntervals;
