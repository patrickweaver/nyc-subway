...
import type { LineName, StationData } from "../types";

// Compare two stations by where they are in the train's route
function compareByLineOrder(
  lineOrder: string[],
  a: StationData,
  b: StationData
): number {
...
function getLineStops(_lineId: string) {
  const lineId: LineName = _lineId.toUpperCase() as LineName;
  // Filter out all stations where train doesn't stop:
  function lineStopsAtStation(stationDataItem: StationData) {
  ...
  const formattedLineStations = lineStations.map((_station) => {
    const station: StationData & { daytimeRoutesArray: string[] | null } = {
      ..._station,
      daytimeRoutesArray: null,
    };
