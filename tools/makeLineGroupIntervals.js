...
async function main() {
  ...
  try {
    ...
    fs.writeFile(
      filename,
      lineGroupIntervalsCopy + JSON.stringify(lineGroupIntervals),
      function (err) {
...
async function getLineGroup(lineGroup) {
  try {
    ...
    const updateStopIds = lineGroupTripEntities
      .map((entity) => {
    ...
    console.log("\n", lineGroup.color);
    const updatedIntervals = updateStopIds
      .flatMap((tripStopIds) => {
    ...
    // Current color:
    lineGroupIntervals[lineGroup.color] = Array.from(
      new Set(
        lineGroupIntervals[lineGroup.color]
          .concat(newIntervals)
